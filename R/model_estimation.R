verticaltry_check_matrix <- function(x, name) {
  x <- as.matrix(x)
  if (!all(is.finite(x))) stop(name, " must contain only finite numeric values.")
  x
}

verticaltry_check_vars <- function(data, vars, label) {
...
verticaltry_prepare_inputs <- function(data = NULL,
                                       time = NULL,
                                       status = NULL,
                                       X = NULL,
                                       Z = NULL,
  ...
  if (!is.null(data)) {
    ...
    total_vars <- verticaltry_check_vars(data, total_vars, "total_vars")
    relative_vars <- verticaltry_check_vars(data, relative_vars, "relative_vars")
    time <- data[[time_var]]
    ...
    X <- verticaltry_check_matrix(X, "X")
    if (is.null(Z)) Z <- X
    Z <- verticaltry_check_matrix(Z, "Z")
...
verticaltry_step_eval0 <- function(x, y, xout) {
  if (length(x) == 0L) return(rep(0, length(xout)))
  stats::stepfun(x, c(0, y), right = TRUE)(xout)
}

verticaltry_smooth_threshold_design <- function(A, W, zeta, h) {
  A <- as.matrix(A)
...
fit_twocause_vertical <- function(time = NULL,
                                  ...
                                  rho_bound = 1.0,
                                  zeta_lr_pi = 1.0) {
  prepared <- verticaltry_prepare_inputs(
...
predict_twocause_vertical <- function(fit, newdata = NULL, times,
  ...
  X <- verticaltry_check_matrix(dat$X, "newdata$X")
  Z <- if (is.null(dat$Z)) X else verticaltry_check_matrix(dat$Z, "newdata$Z")
  W <- as.numeric(dat$W)
  cluster <- as.character(dat$cluster)
  times <- sort(unique(as.numeric(times)))
  train <- fit$training_data

  Xt_train <- verticaltry_smooth_threshold_design(train$X, train$W, fit$zetah, fit$h1)
  ...
  Xt_new <- verticaltry_smooth_threshold_design(X, W, fit$zetah, fit$h1)
  Zt_new <- verticaltry_smooth_threshold_design(Z, W, fit$zetal, fit$h2)
  u_new <- rep(0, nrow(X))
  ...
  for (i in seq_len(nrow(X))) {
    ...
    F1[i, ] <- verticaltry_step_eval0(event_times, Fstep1, times)
    F2[i, ] <- verticaltry_step_eval0(event_times, Fstep2, times)
  }
...
loglik_twocause_vertical <- function(fit) {
  ...
  Xt <- verticaltry_smooth_threshold_design(X, dat$W, fit$zetah, fit$h1)
  ...
  Zt <- verticaltry_smooth_threshold_design(Z, dat$W, fit$zetal, fit$h2)
